...
module.exports = {
   ...
   ADD_FILE: async (req, res) => {
      try {
         const data = new FS(path.resolve(__dirname, '..', '..', '..', '..', 'files', `subCategories.json`))
         const file = JSON.parse(data.read())

         for (const item of file) {
            console.log(item?.title)

            await model.addSubCategoriesFile(
               item?.id,
               item?.title,
               item?.haveItems,
               item?.isNumeric,
               item?.categoryID,
               item?.icon ? item?.icon : "",
               item?.icon ? item?.icon?.split("https://srvr.qiblah.app/public/images/")[1] : "",
            )
         }
